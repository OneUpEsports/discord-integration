...
module.exports.run = async (interaction) => {
  try {
    ...
    if (interaction.customId === "service-ticket") {
      let ticketName = `ticket-${interaction.user.username}`.toLowerCase();
      // let supportRoles = await config.ticketsSupportRoles.map((x) => {
      //   return {
      //     id: x,
      //     allow: [
      //       "VIEW_CHANNEL",
      ...
      if (
        ...
        return interaction.editReply({
          content: `:x: | You already have an open ticket.`,
          ephemeral: true,
        });
      ...
      const embed = new MessageEmbed()
        ...
        .setDescription(
          `Hello <@!${interaction.user.id}>\nHere is your ticket about **Service Support** ...`
        )
        .setColor("RANDOM")
        .setFooter({
